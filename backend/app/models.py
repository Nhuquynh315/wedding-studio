...
class Wedding(db.Model):
    ...
    wedding_tables = db.relationship(
...
class Guest(db.Model):
    ...
    seating_table = db.relationship(
        "WeddingTable", back_populates="guests", foreign_keys=[table_id]
    )
...
class Expense(db.Model):
    ...
    category_id = db.Column(
        db.Integer, db.ForeignKey("budget_categories.id"), nullable=True, index=True
    )
...
class Vendor(db.Model):
    ...
    def to_dict(self):
        return {
            ...
            "deposit_due_date": (
                self.deposit_due_date.strftime("%Y-%m-%d") if self.deposit_due_date else ""
            ),
            "final_payment_amount": (
                self.final_payment_amount if self.final_payment_amount is not None else ""
            ),
            "final_payment_due_date": (
                self.final_payment_due_date.strftime("%Y-%m-%d")
                if self.final_payment_due_date
                else ""
            ),
            "contracted": self.contracted,
            "contract_signed_date": (
                self.contract_signed_date.strftime("%Y-%m-%d") if self.contract_signed_date else ""
            ),
...
class WeddingTable(db.Model):
    ...
    guests = db.relationship("Guest", back_populates="seating_table", foreign_keys="Guest.table_id")
