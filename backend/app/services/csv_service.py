...
def parse_guest_excel(file_obj):
    ...
    for row_num, row in enumerate(rows[1:], start=2):
        ...
        if group_name and group_name not in VALID_GROUPS:
            errors.append(
                f'Row {row_num} ({full_name}): unknown group "{group_name}" — set to blank.'
            )
            group_name = None

        if meal_preference and meal_preference not in VALID_MEALS:
            errors.append(
                f'Row {row_num} ({full_name}): unknown meal "{meal_preference}" — set to blank.'
            )
            meal_preference = None

        if rsvp_status not in VALID_RSVP:
            errors.append(
                f'Row {row_num} ({full_name}): unknown RSVP status "{rsvp_status}" — defaulted to "pending".'
            )
            rsvp_status = "pending"
