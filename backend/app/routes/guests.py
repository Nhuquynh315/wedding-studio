...
from app.routes.utils import get_guest_or_403, get_wedding_or_403
...
def download_excel_template(wedding_id):
    ...
    ws.append(
        ["John Smith", "john@example.com", "+1555010101", "Groom's Family", "Standard", "pending"]
    )
    ws.append(
        ["Jane Doe", "jane@example.com", "+1555010102", "Bride's Family", "Vegetarian", "accepted"]
    )
...
def update_rsvp(guest_id):
    ...
    try:
        ...
        return {
            "ok": True,
            "db_status": db_status,
            "label": _RSVP_LABELS[db_status],
            "color": _RSVP_COLORS[db_status],
        }
    except Exception:
        db.session.rollback()
