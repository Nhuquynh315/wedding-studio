...
from app import db
from app.models import VENDOR_CATEGORIES, VENDOR_STATUSES, BudgetCategory, Expense, Vendor
...
def vendors(wedding_id):
    ...
    all_vendors = (
        Vendor.query.filter_by(wedding_id=wedding_id)
    ...
    budget_categories = (
        BudgetCategory.query.filter_by(wedding_id=wedding_id).order_by(BudgetCategory.name).all()
    )
...
def add_vendor(wedding_id):
    ...
    try:
        ...
        if keywords:
            ...
            has_task = any(
                any(kw in item.title.lower() for kw in keywords) for item in existing_items
            )
            if not has_task:
...
def toggle_final_payment(vendor_id):
    ...
    try:
        ...
        summary = _vendor_summary(vendor.wedding)
        return jsonify(
            {"ok": True, "final_payment_paid": vendor.final_payment_paid, "summary": summary}
        )
    except Exception:
        db.session.rollback()
        return jsonify({"ok": False}), 500
    ...
    if "signed_date" in data:
        ...
        try:
            vendor.contract_signed_date = (
                date_type.fromisoformat(raw_date) if raw_date else date_type.today()
            )
        except ValueError:
        ...
        return jsonify(
            {
                ...
                "signed_date": (
                    vendor.contract_signed_date.isoformat() if vendor.contract_signed_date else ""
                ),
...
def add_to_budget(vendor_id):
    ...
    try:
        db.session.commit()
        return jsonify(
            {"ok": True, "expense_id": expense.id, "category": cat.name if cat else None}
        )
    except Exception:
        db.session.rollback()
        return jsonify({"ok": False, "error": "DB error"}), 500
