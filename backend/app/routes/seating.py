...
from app.routes.utils import get_guest_or_403, get_table_or_403, get_wedding_or_403
...
def seating(wedding_id):
    ...
    tables = (
        WeddingTable.query.filter_by(wedding_id=wedding_id)
        .order_by(WeddingTable.table_number)
        .all()
    )
    unassigned = (
        Guest.query.filter_by(wedding_id=wedding_id, rsvp_status="confirmed")
        .filter(Guest.table_id.is_(None))
        .order_by(Guest.full_name)
        .all()
    )
    accepted_count = Guest.query.filter_by(wedding_id=wedding_id, rsvp_status="confirmed").count()
    accepted_guests = (
        Guest.query.filter_by(wedding_id=wedding_id, rsvp_status="confirmed")
        .order_by(Guest.full_name)
        .all()
    ...
    raw_groups = (
        db.session.query(Guest.group_name)
...
def add_table(wedding_id):
    ...
    max_num_row = (
        db.session.query(db.func.max(WeddingTable.table_number))
...
def auto_assign(wedding_id):
    ...
    unassigned_sorted = sorted(unassigned, key=lambda g: (g.group_name or "zzz", g.full_name))
    ...
    if failed_count > 0:
        message = (
            f"Assigned {assigned_count} guests. {failed_count} could not be placed (tables full)."
        )
    else:
...
def bulk_assign(wedding_id):
    ...
    q = Guest.query.filter_by(wedding_id=wedding_id, rsvp_status="confirmed").filter(
